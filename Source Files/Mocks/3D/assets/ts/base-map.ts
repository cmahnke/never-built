...
export function featurePopUp(feature: Feature | RenderFeature, overlay: Overlay, content: HTMLElement) {
  const geometry = featureFix(feature).getGeometry() as SimpleGeometry;
  const coord = geometry.getCoordinates();
...
function setupMap(element: HTMLElement, geojson: any, source: any, cluster: boolean, marker: any) {
  ...
  // Popup elements
  const container = document.getElementById(element + "-popup");
  const content = document.getElementById(element + "-popup-content");
  const closer = document.getElementById(element + "-popup-closer") as HTMLElement;

  const map = new Map({
      controls: [
        new Zoom({ zoomInTipLabel: toolTips[lang]["zoomIn"], zoomOutTipLabel: toolTips[lang]["zoomOut"] as string }),
...
*/
  ...
  map.getView().fit(vectorSource.getExtent(), { size: map.getSize(), padding: padding });
